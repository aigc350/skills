...
构建 resolved_prompt（平台无关文本描述）:
  ...
  environment = canonical.location.text
...
```text
# 裁决 1：reference_image vs final_prompt
IF consistency.reference_image 存在:
  IF resolved_prompt.subject 与 reference_image 描述冲突:
    → reference_image 优先
    → 修改 resolved_prompt.subject 以匹配
    → 记录 repair_action: "prompt_adjusted_to_reference"
...
IF asset.appearance_fingerprint 存在:
  IF resolved_prompt.subject 与 fingerprint 冲突:
    ...
    → 记录 repair_action: "prompt_overridden_by_fingerprint"

# 裁决 3：seed 继承 vs 新种子
IF temporal.identity_continuity.inherited_seed 存在:
  → 使用 inherited_seed（不生成新种子）
  → 记录 seed_source: "inherited_from_{previous_shot_id}"

# 裁决 4：must_keep 绝对优先
IF consistency.must_keep 中的任何锚点被修改:
  → 恢复 must_keep 值
  → 记录 issue（不可静默修改 must_keep）
```
...
continuity:
  ...
  location_bindings:                    # ✅ 统一命名
    ...
    - 从 asset registry 提取 anchors

  object_bindings:
    - 从 asset 层提取 object_id + asset_id
    - 从 temporal 层提取 object_state
    - 从 asset 层提取 holder
...
构建 referenced_assets（按 priority 排序）:
  ...
  FOR each asset in shot 的所有引用资产:
    - 角色 reference_image → priority = "critical"
    - 场景 reference_image → priority = "high"
    - 物品 reference_image → priority = "medium"
