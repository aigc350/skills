...
**如果文件存在：**
→ 读取配置文件，获取仓库路径
→ 跳转到 [已有配置 - 执行 Git 操作](#已有配置---执行-git-操作) 章节

---

## 已有配置 - 执行 Git 操作

当检测到 `git-config.md` 文件时，直接使用配置执行操作。

### 2.1 读取配置

使用 Read 工具读取 `git-config.md` 文件，解析其中的配置：

- **仓库路径** - 从 "仓库路径" 行提取
- **user.name** - 从配置表提取
- **user.email** - 从配置表提取
- **remote.origin** - 从配置表提取
...
git remote get-url origin 2>/dev/null || echo "NOT_SET"
```
