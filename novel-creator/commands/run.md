...
> **⚠️ 强制约束：禁止跳过任何 Agent 步骤**
>
> 以下规则具有最高优先级，任何情况下不得违反：
> 1. **禁止并行生成多个章节**——必须逐章完成完整循环后，再开始下一章
> 2. **禁止跳过 Editor Agent**（Step 3）——每章必须经过质量检查和字数验证
> 3. **禁止跳过 Recorder Agent**（Step 4）——每章必须更新状态文件
> 4. **禁止批量生成后统一检查**——必须在每章的 Editor 通过后才写下一章
>
> 违反以上约束会导致：字数失控、剧情断裂、设定矛盾。
...
├── Step 3: Editor Agent（质量检查）    ← 禁止跳过
├── Step 4: Recorder Agent（状态更新）  ← 禁止跳过
